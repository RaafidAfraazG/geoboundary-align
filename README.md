...
## Run the boundary-alignment solution

`run_solution.py` keeps the starter baseline available and adds a conservative local alignment
step. It starts from the global median shift when `example_truths.geojson` is present, then tries
small x/y translations for each plot and scores how well the polygon edge lands on `boundaries.tif`.
Large shifts, weak boundary signal, and ambiguous shift choices are penalized. Plots are marked
`corrected` only when the boundary score clearly improves over the original/baseline position;
otherwise they are written as `flagged` with the original geometry.

Run it with uv:

```bash
uv run run_solution.py data/34855_vadnerbhairav_chandavad_nashik
```

On Windows, if `uv` or the global `python` command is not on PATH but the local virtual
environment is already installed, run:

```powershell
.venv\Scripts\python.exe run_solution.py data/34855_vadnerbhairav_chandavad_nashik
```
...
Validate the output shape in PowerShell:

```powershell
$json = Get-Content -Raw data\34855_vadnerbhairav_chandavad_nashik\predictions.geojson | ConvertFrom-Json
$json.type
$json.features.Count
$json.features[0].properties
```

Score the generated file against the public example truths:

```powershell
.venv\Scripts\python.exe -c "from bhume import load, score; import geopandas as gpd; v=load('data/34855_vadnerbhairav_chandavad_nashik'); p=gpd.read_file('data/34855_vadnerbhairav_chandavad_nashik/predictions.geojson'); print(score(p, v))"
```
